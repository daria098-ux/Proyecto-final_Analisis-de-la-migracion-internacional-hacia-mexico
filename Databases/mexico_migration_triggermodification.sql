...
-- Países vinculados a su región respectiva
...
-- Categorías generales como Económica, Forzada, Familiar
CREATE TABLE motive_categories (
    category_id INT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL UNIQUE
);

-- Motivos específicos vinculados a una categoría
...
-- Períodos anuales de registro
CREATE TABLE periods (
    period_id INT PRIMARY KEY AUTO_INCREMENT,
    year YEAR NOT NULL
);

-- Clasificación del nivel socioeconómico de los migrantes
CREATE TABLE socioeconomic_levels (
    level_id INT PRIMARY KEY AUTO_INCREMENT,
    description VARCHAR(100) NOT NULL UNIQUE
);
select * from socioeconomic_levels;
-- Catálogo de riesgos potenciales durante el trayecto
CREATE TABLE risks (
...
-- Catálogo de impactos generados por el fenómeno migratorio
CREATE TABLE impacts (
    impact_id INT PRIMARY KEY AUTO_INCREMENT,
    type ENUM('Social', 'Economic') NOT NULL,
    description VARCHAR(250) NOT NULL
);
-- Tablas de entidades y hechos
-- Registro individual de personas migrantes
...
-- Registro central de eventos migratorios
...
-- 3: Tablas de relaciones de muchos a muchos
-- Relación de riesgos específicos enfrentados en cada migración
...
-- Relación de impactos generados en cada proceso migratorio
...
-- 4: Tablas de estadisticas globales

-- Datos macroeconomicos para la comparacion internacional
...
-- Tabla para el control de cambios y seguridad
CREATE TABLE audit (
...
-- Registro automático de cualquier movimiento de datos en las tablas principales
...
-- Operaciones para Procesos Migratorios
...
-- Operaciones para Estadísticas Globales
...
-- 7: Creacion de vistas analiticas

-- Analiza qué motivos impulsan más la migración hacia México
...
-- Identifica los principales países y regiones que envían migrantes a México
...
-- Comparativa mundial basada en estadísticas externas registradas
...
-- Resumen de los riesgos más frecuentes reportados en territorio nacional
...
-- Clasificación de los impactos Económicos/Sociales detectados en México
...
-- Perfil demográfico detallado (edad, sexo, nivel socioeconómico) de los migrantes
...
-- ----------------------------------------------------
-- Tabla a modificar: Periods

-- Se registra cualquier alta, cambio o eliminacion de un periodo.
CREATE TRIGGER tr_periods_insert
AFTER INSERT ON periods
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('periods', 'Insert', USER());

-- Se encarga de modicar el año de un periodo ya existente.
CREATE TRIGGER tr_periods_update
AFTER UPDATE ON periods
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('periods', 'Update', USER());

-- Se dispara al borrar un periodo, ayuda a detectar aquellos borrados accidentales.
CREATE TRIGGER tr_periods_delete
AFTER DELETE ON periods
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('periods', 'Delete', USER());

-- ------------------------------------------------------------

-- Tabla a modificar: socioeconomic_levels
 
-- Se registra cuando se añade un nuevo nivel socioeconómico
CREATE TRIGGER tr_socioeconomic_levels_insert
AFTER INSERT ON socioeconomic_levels
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('socioeconomic_levels', 'Insert', USER());
 
-- Se registra cuando se edita la descripción de un nivel.
CREATE TRIGGER tr_socioeconomic_levels_update
AFTER UPDATE ON socioeconomic_levels
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('socioeconomic_levels', 'Update', USER());
 
-- Registra cuando se elimina un nivel del catálogo. Si el nivel tiene migrantes asociados, la FK lo bloqueará.
CREATE TRIGGER tr_socioeconomic_levels_delete
AFTER DELETE ON socioeconomic_levels
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('socioeconomic_levels', 'Delete', USER());

-- ------------------------------------------------------------
-- Tabla a modificar: global_statistics
 
-- Se registra la carga de un nuevo dato estadístico.
CREATE TRIGGER tr_global_statistics_insert
AFTER INSERT ON global_statistics
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('global_statistics', 'Insert', USER());
 
-- Se registra correcciones a estadísticas ya cargadas realiza una actualización en world_percentage o total_migrants
CREATE TRIGGER tr_global_statistics_update
AFTER UPDATE ON global_statistics
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('global_statistics', 'Update', USER());
 
-- Se registra la eliminación de un dato estadístico. Borra una fila aquí es inusual si esto ocurre el error debe investigarse.
CREATE TRIGGER tr_global_statistics_delete
AFTER DELETE ON global_statistics
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('global_statistics', 'Delete', USER());
 
-- ------------------------------------------------------------
-- Tabla a modificar: migration_risk
 
-- Se registra cuando se asocia un riesgo a una migración nos permite rastrear qué analista o proceso cargó la información de riesgos para cada caso individual.
CREATE TRIGGER tr_migration_risk_insert
AFTER INSERT ON migration_risk
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('migration_risk', 'Insert', USER());
 
-- Se registra modificaciones en la relación riesgo-migración.
CREATE TRIGGER tr_migration_risk_update
AFTER UPDATE ON migration_risk
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('migration_risk', 'Update', USER());
 
-- Se registra la desvinculación de un riesgo de una migración.
CREATE TRIGGER tr_migration_risk_delete
AFTER DELETE ON migration_risk
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('migration_risk', 'Delete', USER());
 
 
-- ------------------------------------------------------------
-- Tabla a modificar: migration_impact
 
-- Se registra cuando se vincula un impacto a una migración.
CREATE TRIGGER tr_migration_impact_insert
AFTER INSERT ON migration_impact
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('migration_impact', 'Insert', USER());
 
-- Se registra cambios en la relación impacto-migración.
CREATE TRIGGER tr_migration_impact_update
AFTER UPDATE ON migration_impact
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('migration_impact', 'Update', USER());
 
-- Se registrara la eliminación de un impacto vinculado all igual que en migration_risk.
CREATE TRIGGER tr_migration_impact_delete
AFTER DELETE ON migration_impact
FOR EACH ROW
INSERT INTO audit (affected_table, action_type, db_user)
VALUES ('migration_impact', 'Delete', USER());