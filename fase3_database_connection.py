# =============================================================================
# FASE 3 - CARGA A MYSQL (Load)
# Proyecto: Análisis de Migración Internacional hacia México
# =============================================================================
# Lee los CSV limpios de data_clean/ (generados por fase2_transformacion.py)
# e inserta todos los datos en la base de datos mexico_migration.
#
...
#  12. migration_risk        ← clean_missing.csv (vincular por región)
#  13. migration_impact      ← clean_impactos.csv (vincular a migraciones)
# =============================================================================
...
DB_CONFIG = {
    "host":     "localhost",
    "user":     "root",
    "password": "Yaquelin09/",   # >>>AQUI<<< tu contraseña de MySQL
...
def leer_clean(nombre):
    """Lee un CSV limpio de data_clean/."""
...
def ejecutar_muchos(conn, sql, lista_valores):
    """Ejecuta executemany para inserciones masivas."""
    try:
        cu = conn.cursor()
...
def obtener_id(conn, tabla, campo_pk, campo_busqueda, valor):
    """Busca el ID de un registro por su campo de nombre/descripción."""
    try:
        cu = conn.cursor()
        cu.execute(
...
def cache_tabla(conn, sql_all, col_nombre, col_id):
    """Carga una tabla completa en un dict {nombre: id} para evitar
    múltiples SELECT por fila."""
    cu = conn.cursor()
    cu.execute(sql_all)
...
def cargar_motivos(conn):
    ...
    cache_cat = cache_tabla(conn,
        "SELECT name, category_id FROM motive_categories",
        col_nombre=0, col_id=1)
